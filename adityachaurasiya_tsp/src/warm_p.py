...
def build_model(places, distance_matrix, warmstart=True):
    n = len(places)
    model = gp.Model("TSP")
    ...
    # Constraints: Each city must be left and entered exactly once
...
def plot_route(optimal_route, coordinates, places):
    ...
    # Save the map
    m.save('output/tsp_route_warm_p.html')
